...
export default async function sendAlert(type,ingredients){
    try {
        ...
        const existing_document_items = await Alert.find({alert_tag: type, items: {$elemMatch: {name: {$in: ingredients.map(ingredient => ingredient.name)}, expiry_date: ingredients.map(ingredient => ingredient.expiry_date)}}})
