...
export default async function getPAROrder(req, res) {
  ...
  async function PARBuilder(filters) {
    const par_of_all_items = filters.map(async (filter) => {
      ...
      const expiry_period_diff_in_ms = new Date(expiry_date).getTime() - new Date(createdAt).getTime();
      const expiry_period_diff_in_days = Math.floor(expiry_period_diff_in_ms / (1000 * 60 * 60 * 24)) + 1;
