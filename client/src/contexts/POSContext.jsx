...
export default function POSProvider({children}){
    ...
    const removeFromOrder = (id,discount,price) => {
        ...
        if (orders[id] === 1) {
            ...
            localStorage.setItem("orders",JSON.stringify({...orders, [id]: orders[id] - 1}))
        }

        if (Object.keys(JSON.parse(localStorage.getItem("orders"))).length === 0) {
    ...
    const calculateSubTotal = (type,price) => {
        ...
        if (type === 'add' && prevValue){
            newValue = (prevValue + price).toFixed(2)
        }

        else if (type === 'add' && !prevValue) {
            newValue = price
        }
