...
import SpeedOfServiceTable from "../../components/Table/SpeedofServiceTable";
...
export default function SpeedOfService(){
    ...
    const getDefaultReport = async() => {
        const response = await axios.post('/report/speed-of-service',{type:'monthly', month: selectMonth})
    ...
    const handleSearch = async() => {
        if (selectType === 'Daily') {
            const response = await axios.post('/report/speed-of-service',{type:'daily', date: selectDate})
            setReports(response.data.reports)
        }
        else if (selectType === 'Custom Period') {
            const response = await axios.post('/report/speed-of-service',{type:'custom', start_date: startDate, end_date: endDate})
            setReports(response.data.reports)
        }
        else{
          const response = await axios.post('/report/speed-of-service',{type:'monthly', month: selectMonth})
    ...
    return(
        <>
            ...
            <Box sx={mainBoxStyle}>
                ...
                <SpeedOfServiceTable reports={reports} type={selectType}/>
            </Box>
        </>
    );
}