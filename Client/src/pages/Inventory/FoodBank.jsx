...
export default function FoodBank(){
    ...
    const getDefaultReport = async() => {
        const response = await axios.post('/report/foodbank',{type:'monthly', month: selectMonth})
    ...
    const handleSearch = async() => {
        if (selectType === 'Daily') {
            const response = await axios.post('/report/foodbank',{type:'daily', date: selectDate})
            setReports(response.data.reports)
        }
        else if (selectType === 'Custom Period') {
            const response = await axios.post('/report/foodbank',{type:'custom', start_date: startDate, end_date: endDate})
            setReports(response.data.reports)
        }
        else{
          const response = await axios.post('/report/foodbank',{type:'monthly', month: selectMonth})
    ...
    return(
        <>
            ...
            <Query handlePeriod={handlePeriod} select_type={select_type} selectType={selectType} handleSearch={handleSearch}/>
                <Table1 reports={reports} type={selectType} header={'Food Bank'}/>
            </Box>
        </>
    );
}