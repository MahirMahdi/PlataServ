...
export default function Purchases(){
    ...
    const getDefaultReport = async() => {
        const response = await axios.post('/report/purchases',{type:'monthly', month: selectMonth})
    ...
    const handleSearch = async() => {
        if (selectType === 'Daily') {
            const response = await axios.post('/report/purchases',{type:'daily', date: selectDate})
            setReports(response.data.reports)
        }
        else if (selectType === 'Custom Period') {
            const response = await axios.post('/report/purchases',{type:'custom', start_date: startDate, end_date: endDate})
            setReports(response.data.reports)
        }
        else{
          const response = await axios.post('/report/purchases',{type:'monthly', month: selectMonth})
    ...
    return(
        <>
            ...
            <Box sx={mainBoxStyle}>
                ...
                <Table1 reports={reports} type={selectType} header={'Purchases'}/>
            </Box>
        </>
    );
}