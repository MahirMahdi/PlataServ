...
export default function BankingInformation() {
    ...
    const getDefaultReport = async() => {
        const response = await axios.post('/report/bank-info',{type:'monthly', month: selectMonth})
    ...
    const handleSearch = async() => {
        if (selectType === 'Daily') {
            const response = await axios.post('/report/bank-info',{type:'daily', date: selectDate})
            setReports(response.data.reports)
        }
        else if (selectType === 'Custom Period') {
            const response = await axios.post('/report/bank-info',{type:'custom', start_date: startDate, end_date: endDate})
            setReports(response.data.reports)
        }
        else{
          const response = await axios.post('/report/bank-info',{type:'monthly', month: selectMonth})
    ...
    return(
        <>
            ...
            <Box sx={mainBoxStyle}>
                ...
                <TransactionTable type={selectType} reports={reports}/>
            </Box>
        </>
    );
}