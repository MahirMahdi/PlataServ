...
import CashReportTable from "../../components/Table/CashReportTable";
...
export default function CashReport() {
    ...
    const getDefaultReport = async() => {
        const response = await axios.post('/report/cash',{type:'monthly', month: selectMonth})
    ...
    const handleSearch = async() => {
        if (selectType === 'Daily') {
            const response = await axios.post('/report/cash',{type:'daily', date: selectDate})
            setReports(response.data.reports)
        }
        else if (selectType === 'Custom Period') {
            const response = await axios.post('/report/cash',{type:'custom', start_date: startDate, end_date: endDate})
            setReports(response.data.reports)
        }
        else{
          const response = await axios.post('/report/cash',{type:'monthly', month: selectMonth})
    ...
    return(
        <>
            ...
            <Box sx={mainBoxStyle}>
                ...
                <CashReportTable reports={reports} type={selectType}/>
            </Box>
        </>
    );
}